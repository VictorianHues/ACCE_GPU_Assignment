...
extern "C" void do_compute(struct parameters *p, struct results *r) {
    ...
    for (*minute = 0; *minute < p->num_minutes && FLOATING(h_max_spillage_iter) > p->threshold; (*minute)++) {
        ...
        //rainfall_kernel<<<grid, block, rainfall_shared_mem>>>(rows, columns, p->num_clouds,
    ...
    print_matrix(PRECISION_FIXED, rows, columns, h_water_level, "Water after rain");
        ...
        step3_kernel<<<grid, block>>>(d_water_level, d_spillage_flag, d_spillage_level, d_spillage_from_neigh, d_max_spillage_iter,
                                    rows, columns);
...
__global__ void compute_spillage_propagation_kernel(
    ...
    for (int row = row_pos; row < rows; row += stride_row){
        for(int col = col_pos; col < columns; col += stride_col){
            ...
            if (accessMat(spillage_flag, row, col) == 1) {
                accessMat(water_level, row, col) -=
                    FIXED(accessMat(spillage_level, row, col) / SPILLAGE_FACTOR);
            }

                // Compute termination condition: Maximum cell spillage during the iteration
                // Note: atomic max works only for floats 
                int current_spill = FIXED(accessMat(spillage_level, row, col) / SPILLAGE_FACTOR);
                atomicMax(max_spillage_iter, current_spill); 

            for (int cell_pos = 0; cell_pos < CONTIGUOUS_CELLS; cell_pos++) {
                accessMat(water_level, row, col) +=
                    FIXED(accessMat3D(spillage_from_neigh, row, col, cell_pos) / SPILLAGE_FACTOR);
            }
...
extern "C" void do_compute(struct parameters *p, struct results *r) {
    ...
    print_matrix(PRECISION_FIXED, rows, columns, h_water_level, "Water after rain");
        ...
        compute_spillage_propagation_kernel<<<grid_sp, block_sp>>>(d_water_level, d_spillage_flag, d_spillage_level, d_spillage_from_neigh, d_max_spillage_iter, rows, columns);
